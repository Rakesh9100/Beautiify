...
window.addEventListener("scroll", function () {
  ...
  // Update this part to consider dark mode
  if (body.style.backgroundImage === "" || body.style.backgroundImage === "none") {
    scrollToTopButton.style.background = "blue"; // Change this to your desired color
  } else {
...
function changeColor() {
    ...
    if (
      ...
      // Apply changes to content icons
      contentIcons.forEach((icon) => {
        icon.style.color = "white";
      });
  
      // Apply changes to h2 elements inside boxes
      boxText.forEach((h2) => {
        h2.style.color = "white";
      });
  
      // Apply changes to h1 elements inside .contributors
      FootText.forEach((h1) => {
        h1.style.color = "white";
      });
  
      // Apply changes to the start button
